...
for fd in list(list(ko.features())[0].features()): # for each location, make a folder.
    for b in list(fd.features()): # for each placemark, get their building information.
        ...
        pm = kml.Placemark(ns, name=f'{fd.name}_{csuid}_{engBldName}', styleUrl='all')
        pm.geometry = kml.Geometry(geometry=Polygon(withAlt), altitude_mode='absolute', extrude=1)
        f.append(pm)

        bldgpm = kml.Placemark(ns, name=f'{fd.name}_{csuid}_{engBldName}')
        bldgpm.geometry = Point(com[0], com[1])
        # f.append(bldgpm)
        # add this building to CSV.
        thisFolder.append((fd.name, csuid, engBldName, chiBldName, com[0], com[1], pro, baseHeight, baseSource, roofHeight, roofSource, roofHeight-baseHeight, "x"))
