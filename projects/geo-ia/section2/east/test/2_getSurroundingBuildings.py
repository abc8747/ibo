...
probeDistances = (20, 25, 30, 35, 40, 45)
degreeIncrements = 15
...
for counter, placemark in enumerate(list(list(list(ko.features())[0].features())[0].features())):
    # create a folder to store the placemarks
    f = kml.Folder(ns, name=str(counter))
    d.append(f)

    buildings = []
    ln, lt = placemark.geometry.x, placemark.geometry.y
